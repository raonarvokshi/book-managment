...
def get_books():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, title, author_id, book_link, genres, average_rating, published_year FROM books")
...
def create_book(book: BookCreate, _: str = Depends(get_api_key)):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO books (title, author_id, book_link, genres, average_rating, published_year) VALUES (?, ?, ?, ?, ?, ?)", (book.title, book.author_id, book.book_link, genres, book.average_rating, book.published_year))
        conn.commit()
